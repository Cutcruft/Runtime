...
const cfg = useCfg<Scene3DConfig>(props.config, {
  grid: true,
  autoRotate: false
})
...
function materialFor(spec: Scene3DObjectSpec): THREE.MeshStandardMaterial {
  return new THREE.MeshStandardMaterial({
    color: spec.color ? new THREE.Color(spec.color).getHex() : DEFAULT_COLORS[spec.kind]
  })
}
...
function loadContent(content: Scene3DContent): void {
  clearObjects()
  for (const spec of content.objects ?? []) {
    addObject(spec)
  }
}
...
watch(
  ...
  (next) => {
    ...
    if (!scene) return
    try {
      loadContent(JSON.parse(String(next)) as Scene3DContent)
      loaded = true
    } catch {
      /* invalid content, keep empty scene */
    }
  }
)
...
function onPointerUp(e: PointerEvent): void {
  ...
  if (hits.length) {
    ...
    if (specId === null) {
      specId = hits[0].object.userData.specId as string | null ?? null
    }
  }
  selectSpec(specId)
}
...
function setupScene(): void {
  ...
  gridHelper = new THREE.GridHelper(10, 10, 0x94a3b8, 0xdbe3ee)
...
<template>
  <div class="ui-scene3d" :style="height ? { height } : undefined" data-gesture-type="Scene3D">
    <div v-if="toolbar.length" class="ui-scene3d__toolbar">
      <button
        v-for="name in toolbar"
        :key="name"
        class="ui-scene3d__btn"
        :class="{ 'ui-scene3d__btn--disabled': !editable }"
    ...
    <div class="ui-scene3d__canvas" ref="hostEl"></div>
  </div>
</template>
...
.ui-scene3d__canvas :deep(canvas) {
  display: block;
  width: 100%;
  height: 100%;
}
