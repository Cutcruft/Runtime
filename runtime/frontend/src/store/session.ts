...
export const sessionStore = {
  ...
  init(): void {
    ...
    const activeWs = persistedWs ?? deriveWorkspaceFromUrl() ?? 'default'
    workspaceId.value = activeWs
    // WS v2: always carry workspace (+ optional project) in the URL so the server
    // binds the session to the project on connect. Default workspace = "default".
