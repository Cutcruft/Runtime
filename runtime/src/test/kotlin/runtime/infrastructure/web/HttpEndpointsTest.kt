...
class HttpEndpointsTest {
    ...
    private fun withServer(mode: String, block: (Int) -> Unit) {
        val port = freePort()
        val endpoints = HttpEndpoints(HttpConfig(configPath = "/config", staticRoot = "static"), workspace(mode), PluginAssetsService(emptyList()), mode)
